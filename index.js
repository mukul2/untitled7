
const {Base64} = require('js-base64');
const fs = require("fs");
const jo = require('jpeg-autorotate')
const PDFDocument = require("pdfkit");
const blobStream = require('blob-stream');
const { Base64Encode } = require('base64-stream');
const functions = require("firebase-functions");
var bodyParser = require('body-parser');
const express = require('express');
const cors = require('cors');
// Define font files
var fonts = {
    Roboto: {
        normal: 'fonts/Roboto-Regular.ttf',
        bold: 'fonts/Roboto-Medium.ttf',
        italics: 'fonts/Roboto-Italic.ttf',
        bolditalics: 'fonts/Roboto-MediumItalic.ttf'
    }
};

var PdfPrinter = require('pdfmake');
var printer = new PdfPrinter(fonts);
var {google} = require("googleapis");
const fetch = require('node-fetch');
const admin = require('firebase-admin');
const {Firestore} = require('@google-cloud/firestore');
const {Resource} = require('@google-cloud/resource');
const mother_org_id = "964217523348";
const project_api_key = "AIzaSyA_5LFzQb4msoDbY76ZEw0oh-M5gewYeOw";
const nodemailer = require('nodemailer');
let ejs = require('ejs');
var axios = require('axios');
const moment = require('moment');
const {v4: uuidv4} = require("uuid");
//const firebase = require("firebase");
// Required for side-effects
//require("firebase/firestore");
var superEmail = "" ;
var superKey = "";
var fontRegular ;
var fontBold;
let serviceAccount = require('./key.json');
let rawdata = fs.readFileSync('./key.json');
//let rawdatahtml = fs.readFileSync('action.html', "utf8");
let jsonData = JSON.parse(rawdata);
superEmail = jsonData.client_email;
superKey = jsonData.private_key;

admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
});

async function createReport(res,customerInfo,reportID,isTest){
    console.log("before createReport");
    function savePdf(firestore,storage,data) {
        var tok  = uuidv4();

        try{
            const metadata = {
                metadata: {
                    // This line is very important. It's to create a download token.
                    firebaseStorageDownloadTokens:tok
                },
                contentType: 'auto',
                cacheControl: 'public, max-age=31536000',
            };
            //var fileName =  customerInfo.get("projectId")+"/reports/"+reportID+".pdf";
            var name =reportID+".pdf";

            // gcFile = storage.bucket('gs://staht-connect-322113.appspot.com').file(name)


            const myBucket = storage.bucket('gs://staht-connect-322113.appspot.com');
            const blob = myBucket.file("reports/"+name);

            buffer = Buffer.from(data, 'base64');
            const blobStream = blob.createWriteStream({
                resumable: false,
                validation : false,

                contentType: "auto",
                metadata   : metadata
            });
            blobStream.end(buffer);




            // buffer = Buffer.from(data, 'base64');
            // const blobStream = gcFile.createWriteStream({
            //     resumable: false,
            //     validation : false,
            //     contentType: "auto",
            //     metadata   : metadata
            // });
            // blobStream.end(buffer);
            // gcFile.makePublic();
            var linkOfImg = "https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/reports%2F"+name+"?alt=media&token="+tok;

            // var linkOfImg = "https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/"+"%2F"+ customerInfo.get("projectId")+"%2F"+ "reports"+"%2F"+name+"?alt=media&token="+tok;
            firestore.collection("reports").doc(reportID).update({"pdfFile": linkOfImg});
            console.log("pdf Saved");
            console.log(linkOfImg);
            //res.send(linkOfImg);
            res.set('Access-Control-Allow-Origin', '*');
            res.set('Access-Control-Allow-Methods', 'GET, POST');
            res.send({"link":linkOfImg});

            // console.log(data);
            //  res.send(data);



        }catch (e) {
            res.set('Access-Control-Allow-Origin', '*');
            res.set('Access-Control-Allow-Methods', 'GET, POST');
            console.log("Failed");
            console.log(e);
            //res.send(e);
            res.send({"status":false});
        }
    }
    function getMainStorage(customerFirebase,data) {
        var project_idMain = 'staht-connect-322113';
        try{
            admin.initializeApp({
                //credential: admin.credential.cert(serviceAccount)
                credential: admin.credential.cert({
                    privateKey : superKey,
                    clientEmail : superEmail, // privateKey: "-----BEGIN PRIVATE KEY-----\\ns9o2ZsCf89CTRWexlVA9ibon\n-----END PRIVATE KEY-----\n",
                    //  clientEmail:  "allpermissions@mindful-vial-304611.iam.gserviceaccount.com",
                    projectId: project_idMain,
                })


            },project_idMain);
            const storage= admin.app(project_idMain).storage();
            savePdf(customerFirebase,storage,data);
        }catch (e) {
            const storage= admin.app(project_idMain).storage();
            savePdf(customerFirebase,storage,data);

        }

    }

    let doc = new PDFDocument({ size: "A4", margin: 50 });


    var linkBold = "https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/Calibri%20Bold.TTF?alt=media&token=5f9cebeb-8448-4b75-b6ac-c6ee600f5790";
    try{
        fontRegular =  await axios.get("https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/Calibri%20Regular.ttf?alt=media&token=666e58a3-c099-472e-a9e5-9597faa612f6", {responseType: 'arraybuffer'})
        fontBold =  await axios.get(linkBold, {responseType: 'arraybuffer'})

        doc.font(fontRegular.data);
    }catch (err) {
        console.log("font load error");
        console.log(err);


    }

    console.log("before generateHeader");


    var respo =  await    generateHeaderVJTEch(doc,customerInfo,reportID,isTest);
    var dd2 = {
        content: [
            'First paragraph',
            'Another paragraph, this time a little bit longer to make sure, this line will be divided into at least two lines'
        ]

    }

    var externalDataRetrievedFromServer = respo["tests"];

    function buildTableBody(data, columns) {
        var body = [];

        //body.push(columns);

        data.forEach(function(row) {
            var dataRow = [];

            columns.forEach(function(column) {
                dataRow.push(row[column].toString());
            })

            // columns.forEach(function(column) {
            //     dataRow.push(row[column].toString());
            // })

            body.push(dataRow);
        });

        return body;
    }

    function table(data, columns) {
        return {
            table: {
                headerRows: 1,  widths: [40,100,50,80,230],
                body: buildTableBody(data, columns)
            }
        };
    }
    var dd = {
        content: [

            {
                table: {
                    layout: 'noBorders',
                    widths: [100, '*', 100, ],
                    body: [

                        [
                            {text: 'Logo Here', alignment:'center',style :{italics:true,fontSize: 14,}},
                            // {
                            //     image: 'data:image/jpeg;base64,',
                            //     width: 100,
                            //     alignment:'center'
                            // },
                            {
                                stack: [

                                    {text: 'Fixing Test Report', alignment:'center',style :{italics:true,fontSize: 28,}},
                                    {text:respo["companyName"]+", "+respo["address"], alignment:'center',style :{italics:true,fontSize: 8,}},
                                ],
                                style: 'header'
                            },

                            {text: 'Logo Here', alignment:'center',style :{italics:true,fontSize: 14,}},
                            // {
                            //     image: 'data:image/jpeg;base64,',
                            //     width: 100,
                            //     alignment:'center'
                            // },

                        ]
                    ]
                },
                layout: 'noBorders',
                style: 'header'
            },



            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [90,180,80,130],

                    // keepWithHeaderRows: 1,
                    body: [
                        ['Site test report',respo["reportTitle"]  , 'Date of test',respo["reportDate"]],
                        [{colSpan: 1,text:'Report code' }, {colSpan: 3,text:'VJTASH TR VJT 22-Jun-22 RR22062022AG' }],




                    ]
                }
            },
            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [163,163,163],
                    body: [
                        [{border: [true, false, true, true],colSpan: 1,text:'Client details' },
                            {border: [true, false, true, true],colSpan: 1,text:'Site details' },
                            {border: [true, false, true, true],colSpan: 1,text:'Witnessed by' },
                        ],
                        [{colSpan: 1,text:respo["clientDetails"]},
                            {colSpan: 1,text:respo["siteDetails"]},
                            {colSpan: 1,text:respo["witness"] }
                        ],

                    ]
                }
            },
            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [120,120,120,120],
                    body: [
                        [{border: [true, false, true, true],colSpan: 1,text:'Test conducted by' }, {border: [true, false, true, true],colSpan: 3,text:respo["testConductedBy"] },],
                        [{colSpan: 1,text:'Fixing type' }, {colSpan: 3,text:respo["fixingType"]  },],
                        [{colSpan: 1,text:'Resin type' }, {colSpan: 3,text:respo["resinType"] },],
                        [{colSpan: 1,text:'Base material' }, {colSpan: 3,text:respo["baseMaterial"] },],
                        [{colSpan: 1,text:'Proof/ultimate/prelim' }, {colSpan: 3,text:respo["proff"] },],


                    ]
                }
            },
            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [120,120,120,120],
                    body: [

                        [{border: [true, false, true, true],colSpan: 1,text:'Hole diameter (mm)' }, {border: [true, false, true, true],colSpan: 1,text:respo["holeDia"] },{border: [true, false, true, true],colSpan: 1,text:'Edge distance (mm)' }, {border: [true, false, true, true],colSpan: 1,text:respo["edDistance"] },],
                        [{colSpan: 1,text:'Hole depth (mm)' }, {colSpan: 1,text:respo["holeDepth"] },{colSpan: 1,text:'Anchor centres (mm)' }, {colSpan: 1,text:respo["anckorCent"] },],


                    ]
                }
            },
            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [220,30,190,40],
                    body: [

                        [{	border: [true, false, true, true],text:'Recommended embedment depth (mm)' }, {border: [true, false, true, true],text:respo["reEmD"] },{border: [true, false, true, true],text:'Actual embedment depth (mm)' }, {border: [true, false, true, true],text:respo["acTEmbDepth"] },],
                        [{text:'Temperature when installed (°C)' }, {text:respo["temp"] },{text:'Curing time (if applicable)' }, {text:respo["curingT"] },],


                    ]
                }
            },

            {
                style: 'tableExample',
                color: '#444',
                table: {
                    widths: [163,163,163],
                    body: [

                        [{	border: [true, false, true, true],text:'Test Equipment' }, {border: [true, false, true, true],text:"Model No" },
                            {border: [true, false, true, true],text:'Serial No' }],
                        [{text:respo["testEquip"] }, {text:respo["modelNo"] },{text:respo["serialNo"]}],

                        [{	border: [true, false, true, true],text:'Gauge No' }, {border: [true, false, true, true],text:"Gauge Range (kN)" },
                            {border: [true, false, true, true],text:'Date of calibration' }],
                        [{text:respo["gaugeNo"] }, {text:respo["gaugeRange"] },{text:respo["dateOfCalibration"]}],


                    ]
                }
            },

            {
                style: 'tableExampleBr',
                color: '#444',
                table: {

                    widths: [507],
                    body: [

                        [{	border: [true, true, true, true],text:'Additional comments (indicate any deviations from the standard test method here):' ,color: '#000'}],


                        [{text:respo["additional"] }],


                    ]
                }
            },

            {  pageBreak: 'before',
                table: {

                    layout: 'noBorders',
                    widths: [100, '*', 100, ],
                    body: [

                        [
                            {text: 'Logo Here', alignment:'center',style :{italics:true,fontSize: 14,}},
                            // {
                            //     image: 'data:image/jpeg;base64,',
                            //     width: 100,
                            //     alignment:'center'
                            // },
                            {
                                stack: [

                                    {text: 'Fixing Test Report', alignment:'center',style :{italics:true,fontSize: 28,}},
                                    {text:respo["companyName"]+", "+respo["address"], alignment:'center',style :{italics:true,fontSize: 8,}},
                                ],
                                style: 'header'
                            },

                            {text: 'Logo Here', alignment:'center',style :{italics:true,fontSize: 14,}},
                            // {
                            //     image: 'data:image/jpeg;base64,',
                            //     width: 100,
                            //     alignment:'center'
                            // },

                        ]
                    ]
                },
                layout: 'noBorders',
                style: 'header'
            },

            //'Prel. (kN)','Ultimate (kN)','Comments (1st movement / failure mode)'

            table(externalDataRetrievedFromServer, ['TestNo', 'Sampleinfo',"Prel","Ultimate","Comments"])
        ],
        styles: {

            tableExampleBr: {
                margin: [0, 8,0, 0]
            },

        },
    }

    var pdfDoc = printer.createPdfKitDocument(dd);
    console.log("Gone here");

    // pdfDoc.pipe(fs.createWriteStream('document.pdf'));
    // pdfDoc.end();
    // console.log("saved");
  if(true)  try{
        var pdfDoc = printer.createPdfKitDocument(dd);
        console.log("Gone here");



        pdfDoc.pipe(new Base64Encode());
        console.log("Gone here 2");

        var finalString = ''; // contains the base64 string
        var stream = pdfDoc.pipe(new Base64Encode());
        console.log("Gone here 3");
        pdfDoc.end();
        console.log("Gone here 4");
        stream.on('data', function(chunk) {
            finalString += chunk;
            console.log("Gone here data");
        });

        stream.on('end', function() {
            console.log("Gone here end");
            // the stream is at its end, so push the resulting base64 string to the response
            //  console.log(finalString);
            //upload the file to storage
            //make pdf
            var bin = Base64.atob(finalString);
// Your code to handle binary data
            fs.writeFile('result_binary.pdf', bin, 'binary', error => {
                if (error) {
                    throw error;
                } else {
                    console.log('binary saved!');
                }
            });
            //make pdf ends

            /*

             var projId = customerInfo.get("projectId");
             try{
                 admin.initializeApp({
                     //credential: admin.credential.cert(serviceAccount)
                     credential: admin.credential.cert({
                         privateKey : superKey,
                         clientEmail : superEmail, // privateKey: "-----BEGIN PRIVATE KEY-----\\ns9o2ZsCf89CTRWexlVA9ibon\n-----END PRIVATE KEY-----\n",
                         //  clientEmail:  "allpermissions@mindful-vial-304611.iam.gserviceaccount.com",
                         projectId: projId,
                     })


                 },projId);

                 const CUstomerfireStore= admin.app(projId).firestore();
                 getMainStorage(CUstomerfireStore,finalString);



             }catch (e) {

             */
            /*
                const CUstomerfireStore= admin.app(projId).firestore();
                getMainStorage(CUstomerfireStore,finalString);

             */


            //  }



        })



    }catch (e) {
        console.log(e);

    }


    // generateCustomerInformation(doc, invoice);
    // generateInvoiceTable(doc, invoice);
    // generateFooter(doc);



}

async function generateHeaderVJTEch(doc,customerInfo,reportID,isTest) {

    var dataObject = {};




    function millisToMinutesAndSeconds(millis) {
        var minutes = Math.floor(millis / 60000);
        var seconds = ((millis % 60000) / 1000).toFixed(0);
        return minutes + ":" + (seconds < 10 ? '0' : '') + seconds;
    }
    function secondsToMinutesAndSeconds(millis) {
        var minutes = Math.floor(millis / 60);
        var seconds = ((millis % 60000) / 1).toFixed(0);
        return minutes + ":" + (seconds < 10 ? '0' : '') + seconds;
    }
    function createTableU(mainCompanyName,testTitle,startingY,doc, dataMother, width = doc.page.width-100) {
        const startY = startingY;
        const  startX = 50;
        let distanceY = 20;
        let distanceX = 10;

        doc.fillColor("grey");
        doc.fontSize(12);
        doc.lineWidth(0.005);
        let currentY = startY;
        let currentX = startX;
        var heightYY = 15 ;

        var breakNeeded = 0 ;

        for(var i = 0  ; i <dataMother.length ; i++ ){

            var data = dataMother[i];

            currentX = 50;
            //currentY = currentY + 10;

            for(var j = 0  ; j <data.length ; j++ ){
                var value = data[j];
                {
                    // currentX = 50;
                    // var dataValue =  value[1].replace(/(\r\n|\n|\r)/gm, "");
                    var dataValue =  value[1].replace(/\n\s*\n/g, '\n');

                    dataValue = dataValue.replace(/\n\s*\n\s*\n/g, '\n');

                    var breakCount = dataValue.split(/\r\n|\r|\n/).length;
                    if(dataValue.length<60)distanceY = 25;
                    else  if(dataValue.length<120)distanceY = heightYY*2+(10);
                    else  if(dataValue.length<180)distanceY = heightYY*3+(10);
                    else  if(dataValue.length<240)distanceY = heightYY*4+(10);
                    else  if(dataValue.length<300)distanceY = heightYY*5+(10);
                    else  if(dataValue.length<360)distanceY = heightYY*6+(10);
                    else  if(dataValue.length<420)distanceY = heightYY*7+(10);
                    else  if(dataValue.length<480)distanceY = heightYY*8+(10);
                    else  if(dataValue.length<520)distanceY = heightYY*9+(10);
                    else  if(dataValue.length<(520+(60*1)))distanceY = heightYY*10+(10);
                    else  if(dataValue.length<(520+(60*2)))distanceY = heightYY*11+(10);
                    else  if(dataValue.length<(520+(60*3)))distanceY = heightYY*12+(10);
                    else  if(dataValue.length<(520+(60*4)))distanceY = heightYY*13+(10);
                    else  if(dataValue.length<(520+(60*5)))distanceY = heightYY*14+(10);
                    else  if(dataValue.length<(520+(60*6)))distanceY = heightYY*15+(10);


                    else {
                        //distanceY = heightYY*16+(10)
                    };




                    distanceX = 10;

                    size = value.length;
                    let nowprinting = 0;

                    let blockSize = width / size;
                    var factorSlicer = 1 ;

                    if(data.length==1){

                    }else   if(data.length==2){
                        factorSlicer = 2 ;
                    }
                    value.forEach(text => {
                        if(nowprinting == 0){



                            blockSize = width*0.33/factorSlicer;
                            nowprinting ++ ;
                        }else{
                            text = dataValue;
                            blockSize = width*0.67/factorSlicer;
                            nowprinting ++ ;
                        }
                        //Write text


                        if(currentY>750 ||((currentY + distanceY) > 750) ){

                            currentY = 80;
                            doc.addPage();
                            doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
                            doc.fontSize(12);
                            doc.fillColor('grey');
                            doc
                                .lineWidth(0.005);

                            doc.strokeColor("grey");


                            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
                            //    doc.fillColor('black').text('Report link', 20, 50, {});
                            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});
                            doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                                lineBreak: false
                            });
                        }


                        if(nowprinting == 1){





                            doc.font(fontBold.data);
                            doc.fillColor("grey");

                            var oldText = text;
                            if(true || oldText.length>25){
                                doc.text(oldText,currentX + distanceX, currentY+8, { width: blockSize*0.95 , align: "left"});


                                // var a = oldText;
                                // var b = "\n";
                                // var position = 25;
                                // var output = [a.slice(0, position), b, a.slice(position)].join('');
                                //
                                // doc.text(output, currentX + distanceX, currentY+8);
                            }else{
                                //currentY = currentY +8;
                                doc.text(text, currentX + distanceX, currentY+8);
                            }



                        }else{
                            //currentY = currentY +8;
                            doc.fillColor("black");
                            doc.font(fontRegular.data);
                            doc.fillColor("#000000");
                            doc.text(text, currentX + distanceX, currentY+8);






                        }




                        //   doc.fillColor("red");
                        doc.strokeColor("grey");
                        //Create rectangles
                        doc
                            .lineJoin("miter")
                            .rect(currentX, currentY, blockSize, distanceY)
                            .stroke()
                            .lineWidth(0.005)
                            .strokeColor("grey");

                        currentX += blockSize;


                    });
                    if(j+1 ==data.length ){
                        currentY = currentY +heightYY ;
                        if(breakCount>1){

                            breakNeeded = distanceY + breakCount*heightYY;
                            currentY =currentY+ distanceY + breakCount*heightYY;
                        }else if(value[0].length > 26){
                            // distanceY = distanceY + 1*heightYY;
                            breakNeeded = distanceY + 1*heightYY;
                            //distanceY = heightYY*2+(10);
                            // currentY = currentY +heightYY ;
                            currentY =currentY+ distanceY + 1*heightYY;
                        }
                    }
                    //currentY += distanceY;



                }
                if(true){

                }
            }

            data.forEach(value =>{} );


        }


        dataMother.forEach(data=>{


        });






        if(currentY>750){

            currentY = 80;

            doc.addPage();
            doc
                .lineWidth(0.005);

            doc.strokeColor("grey");
            doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
            doc.fontSize(12);
            doc.fillColor('grey');

            doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

            if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});




            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
            //  doc.fillColor('black').text('Report link', 20, 50, {});

            // doc.text('Signature', 60,currentY, {
            //     lineBreak: false
            // });

            //   if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),110,currentY, { height: 50 , align: "center"});
            doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                lineBreak: false
            });
            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});


        }else{
            currentY =   currentY+50 ;
        if(false)    doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

            if(false)   if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});


            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});

        }


    }
    function createTable(mainCompanyName,testTitle,startingY,doc, data, width = doc.page.width-100) {
        const startY = startingY;
        const  startX = 50;
        let distanceY = 20;
        let distanceX = 10;

        doc.fillColor("grey");
        doc.fontSize(12);
        doc.lineWidth(0.005);
        let currentY = startY;

        var heightYY = 15 ;

        data.forEach(value => {

            // var dataValue =  value[1].replace(/(\r\n|\n|\r)/gm, "");
            var dataValue =  value[1].replace(/\n\s*\n/g, '\n');

            dataValue = dataValue.replace(/\n\s*\n\s*\n/g, '\n');

            var breakCount = dataValue.split(/\r\n|\r|\n/).length;
            if(dataValue.length<60)distanceY = 25;
            else  if(dataValue.length<120)distanceY = heightYY*2+(10);
            else  if(dataValue.length<180)distanceY = heightYY*3+(10);
            else  if(dataValue.length<240)distanceY = heightYY*4+(10);
            else  if(dataValue.length<300)distanceY = heightYY*5+(10);
            else  if(dataValue.length<360)distanceY = heightYY*6+(10);
            else  if(dataValue.length<420)distanceY = heightYY*7+(10);
            else  if(dataValue.length<480)distanceY = heightYY*8+(10);
            else  if(dataValue.length<520)distanceY = heightYY*9+(10);
            else  if(dataValue.length<(520+(60*1)))distanceY = heightYY*10+(10);
            else  if(dataValue.length<(520+(60*2)))distanceY = heightYY*11+(10);
            else  if(dataValue.length<(520+(60*3)))distanceY = heightYY*12+(10);
            else  if(dataValue.length<(520+(60*4)))distanceY = heightYY*13+(10);
            else  if(dataValue.length<(520+(60*5)))distanceY = heightYY*14+(10);
            else  if(dataValue.length<(520+(60*6)))distanceY = heightYY*15+(10);


            else distanceY = heightYY*16+(10);

            if(breakCount>1){
                distanceY = distanceY + breakCount*heightYY;
            }else if(value[0].length > 26){
                distanceY = distanceY + 1*heightYY;

                //distanceY = heightYY*2+(10);
                //currentY = currentY +heightYY ;
            }

            distanceX = 10;
            let currentX = startX,
                size = value.length;
            let nowprinting = 0;

            let blockSize = width / size;

            value.forEach(text => {
                if(nowprinting == 0){
                    blockSize = width*0.33;
                    nowprinting ++ ;
                }else{
                    text = dataValue;
                    blockSize = width*0.67;
                    nowprinting ++ ;
                }
                //Write text


                if(currentY>750 ||((currentY + distanceY) > 750) ){

                    currentY = 80;
                    doc.addPage();
                    doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
                    doc.fontSize(12);
                    doc.fillColor('grey');
                    doc
                        .lineWidth(0.005);

                    doc.strokeColor("grey");


                    //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
                    //    doc.fillColor('black').text('Report link', 20, 50, {});
                    doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});
                    doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                        lineBreak: false
                    });
                }


                if(nowprinting == 1){





                    doc.font(fontBold.data);
                    doc.fillColor("black");

                    var oldText = text;
                    if(true || oldText.length>25){
                        doc.text(oldText,currentX + distanceX, currentY+8, { width: blockSize*0.95 , align: "left"});


                        // var a = oldText;
                        // var b = "\n";
                        // var position = 25;
                        // var output = [a.slice(0, position), b, a.slice(position)].join('');
                        //
                        // doc.text(output, currentX + distanceX, currentY+8);
                    }else
                        doc.text(text, currentX + distanceX, currentY+8);
                }else{
                    doc.font(fontRegular.data);
                    doc.fillColor("#000000");
                    doc.text(text, currentX + distanceX, currentY+8);






                }




                //   doc.fillColor("red");
                doc.strokeColor("grey");
                //Create rectangles
                doc
                    .lineJoin("miter")
                    .rect(currentX, currentY, blockSize, distanceY)
                    .stroke()
                    .lineWidth(0.005)
                    .strokeColor("grey");

                currentX += blockSize;


            });

            currentY += distanceY;



        });



        if(currentY>750){

            currentY = 80;
            doc.addPage();
            doc
                .lineWidth(0.005);

            doc.strokeColor("grey");
            doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
            doc.fontSize(12);
            doc.fillColor('grey');

            doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

            if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});




            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
            //  doc.fillColor('black').text('Report link', 20, 50, {});

            // doc.text('Signature', 60,currentY, {
            //     lineBreak: false
            // });

            //   if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),110,currentY, { height: 50 , align: "center"});
            doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                lineBreak: false
            });
            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});


        }else{
            currentY =   currentY+50 ;
            doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

            if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});


            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});

        }


    }
    function dateTime(argument,zone) {
        var time= moment.utc(argument ).local().format('DD MMM YYYY HH:mm');

        if(zone){
            time = moment(argument).zone(zone).format('DD MMM YYYY HH:mm');
        }else{
            // var summerTimeEnds = 1635645600;

            // if(argument>summerTimeEnds){
            //      time = moment(argument).zone(60).format('DD MMM YYYY HH:mm');
            // }else{
            //     time = moment(argument).zone(-60).format('DD MMM YYYY HH:mm');
            // }


        }

        // var time = moment(argument).zone(0).format('DD MMM YYYY HH:mm');
        return time;

// var options = {year: 'numeric', month: 'long', day: '2-digit', hour: '2-digit', minute: '2-digit', };
// var d = new Date(argument);
// var ds = d.toLocaleDateString("en-US", options);
// return ds;
    }
    function textInRowFirst(doc, text,val, heigth) {
        doc.y = heigth+10;
        doc.x = 30;
        doc.fillColor('black')
        doc.text(text, {
            paragraphGap: 5,
            indent: 5,
            align: 'left',
            columns: 1,
        });
        doc.y = heigth+10;
        doc.x = 200;
        doc.fillColor('black')
        doc.text(val, {
            paragraphGap: 5,
            indent: 5,
            align: 'right',
            columns: 1,
        });
        return doc
    }


    function row(doc, heigth) {
        doc.lineJoin('miter')
            .rect(30, heigth, doc.page.width-70, 30)
            .stroke()
        return doc
    }
    function rowBig(doc, heigth,level) {
        doc.lineJoin('miter')
            .rect(30, heigth, doc.page.width-60, 12+(18*(level)))
            .stroke()
        return doc
    }
//doc.image((new Buffer(image.replace('data:image/png;base64,',orgLogoBase64), 'base64'), 100, 100));

// Pipe its output somewhere, like to a file or HTTP response
// See below for browser usage
//doc.pipe(fs.createWriteStream('output.pdf'));

// Embed a font, set the font size, and render some text

    const firestoreDb = admin.firestore();



    //seperate db
    console.log(customerInfo.get("projectId"));


    try{




        admin.initializeApp({
            //credential: admin.credential.cert(serviceAccount)
            credential: admin.credential.cert({
                privateKey : superKey,
                clientEmail : superEmail,
                // privateKey: "-----BEGIN PRIVATE KEY-----\\ns9o2ZsCf89CTRWexlVA9ibon\n-----END PRIVATE KEY-----\n",
                //  clientEmail:  "allpermissions@mindful-vial-304611.iam.gserviceaccount.com",
                projectId: customerInfo.get("projectId"),
            })


        }, customerInfo.get("projectId"));
    }catch (err) {
        console.log("admin initialize error");

        //res.send("Could not open Database");
    }
    const firestoreDbCustomer= admin.app( customerInfo.get("projectId")).firestore();

    //seperate db end


    var conclutionLevel = 1 ;

    const reportDetails = await firestoreDbCustomer.collection('reports').doc(reportID).get();
    console.log(1);
//reportDetails.get("field_11")
    if(reportDetails.get("field_11")){
        if(reportDetails.get("field_11").length > 60)  conclutionLevel = 2;
        if(reportDetails.get("field_11").length > 120)  conclutionLevel = 3;
        if(reportDetails.get("field_11").length > 120)  conclutionLevel = 4;
        if(reportDetails.get("field_11").length > 180)  conclutionLevel = 5;
        if(reportDetails.get("field_11").length > 240)  conclutionLevel = 6;
        if(reportDetails.get("field_11").length > 300)  conclutionLevel = 7;
        if(reportDetails.get("field_11").length > 360)  conclutionLevel = 8;
        if(reportDetails.get("field_11").length > 420)  conclutionLevel = 9;

    }else{
        conclutionLevel =1;
    }
    var reporterName = "Empty";


    if(firestoreDb){
        console.log(2);


        var cs = await firestoreDb.collection("users").where("uid","==",reportDetails.get("report_by")).get();


        for (var i = cs.docs.length - 1; i >= 0; i--) {
            reporterName = cs.docs[i].get("displayName");
        }



//   const reporter = await firestoreDb.collection('users').where("uid","==",reportDetails.get("report_by")).get();

// reporter.forEach((doc) => {
//   // doc.data() is never undefined for query doc snapshots
//   console.log(doc.id, " => ", doc.data());
//   //reporterName = doc.get("displayName");
// });
    }


    var  logo =  customerInfo.get("logo");
    var  address =  customerInfo.get("address").toUpperCase();
    var  telephone =  customerInfo.get("telephone");
    var  prjectName =  customerInfo.get("prjectName").toUpperCase();
    var  email =  customerInfo.get("email");

    dataObject["companyName"] = prjectName;
    dataObject["address"] = address;
    doc.moveDown();
    doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
        lineBreak: false
    });
    // .image(new Buffer(logo.replace('data:image/png;base64,',''), 'base64'),doc.page.width-130,10,  {fit: [130, 130], align: 'right', valign: 'center'})
    if(customerInfo.get("logo")){
        doc
            .image(new Buffer(logo.replace('data:image/png;base64,',''), 'base64'),doc.page.width-130-50, 10,{fit: [130, 130], align: 'right', valign: 'bottom'})
            .fillColor("#000000")
            .fontSize(15)
            .text(prjectName, 0, 150,{ align: "right" })
            .fontSize(12)
            .fillColor("#000000")
            .text(address, 0, 170, { align: "right" })
            .fontSize(12)
            .text(email, 0, 192, { align: "right" })
            .fontSize(12)
            .text(telephone, 0, 210, { align: "right" })
            .moveDown();
    }else{

        var im =  await axios.get("https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/utils-photos%2Fcfa_vj.png?alt=media&token=2b8df549-af35-4556-b6eb-f13d033de465", {responseType: 'arraybuffer'});
        var im2 =  await axios.get("https://firebasestorage.googleapis.com/v0/b/staht-connect-322113.appspot.com/o/utils-photos%2Flogo%20(9).png?alt=media&token=112e974b-5c91-450f-81f4-480e9b0a7178", {responseType: 'arraybuffer'});
        doc
                 .image(new Buffer(im.data, 'base64'),doc.page.width-100-50, 10,{fit: [100, 100], align: 'left', valign: 'bottom'})
                 .image(new Buffer(im2.data, 'base64'),50, 10,{fit: [100, 100], align: 'right', valign: 'bottom'})

            .fillColor("#000000")
            .fontSize(20)
            .text("Fixing Test Report", 50, 50,{ align: 'center', valign: 'center'})
            .fontSize(10)
            .fillColor("#000000").moveDown()
            .text(prjectName+", "+address, (doc.page.width-300)/2, 70, {  width: doc.page.width-300,  align: 'center', valign: 'center'})
            .fontSize(12).moveDown();
           // .text(email,  {fit: [100, 300], align: 'center', valign: 'center'})
           // .fontSize(12).moveDown()
         //   .text(telephone,  { align: 'center', valign: 'center'})
           // .moveDown();
    }

  if(false)  createTableU(prjectName,"Test title",130,doc,[
        [["Site test report","Tensile loading - CFA compliant"],["Date of test","22 June 2022"]],
        [["Report code","VJTASH TR VJT 22-Jun-22 RR22062022AG"]],
        [["Client details","Advanced Glass Ltd\n" +
        "No 6 Watling Close\n" +
        "Sketchley Meadows Industrial\n" +
        "Estate\n" +
        "Hinckley\n" +
        "LE10 3EZ"],["Site details","Brent Cross Station\n" +
        "Brent Terrace\n" +
        "London\n" +
        "NW2 1LN"]],



    ]);
// doc.lineCap('butt')
//   .moveTo(200, 310-60)
//   .lineTo(200, 400-60)
//   .stroke()

    // doc.lineCap('butt')
    // .moveTo(200, 470-60)
    // .lineTo(200, 710-60+(conclutionLevel-1)*18)
    // .stroke()


// row(doc, 310-60);
// row(doc, 340-60);
// row(doc, 370-60);


// row(doc, 470-60);

// row(doc, 500-60);
// row(doc, 530-60);
// row(doc, 560-60);
// row(doc, 590-60);
// row(doc, 620-60);
// row(doc, 650-60);
// rowBig(doc, 680-60,conclutionLevel);
    doc
        .lineWidth(0.005);

    doc.strokeColor("grey");
    doc.fontSize(12);

// textInRowFirst(doc, 'Report Title',reportDetails.get("field_1"), 310-60);
// textInRowFirst(doc, 'Report Date',dateTime(reportDetails.get("created_at")), 340-60);
// textInRowFirst(doc, 'Compiled By',reporterName, 370-60);




    if(reportDetails.get("field_1")){

        dataObject["reportTitle"] = reportDetails.get("field_1");
        dataObject["reportDate"] = dateTime(reportDetails.get("created_at"));



      if(false)  createTable(prjectName,reportDetails.id,130,doc ,[
            [ 'Report Title',reportDetails.get("field_1")],
            ['Report Date',dateTime(reportDetails.get("created_at"))],
            ['Compiled By',reporterName],
            [ 'Customer Company Name',reportDetails.get("field_2")],
            ['Customer Address',reportDetails.get("field_5")],
            ['Site Name',reportDetails.get("field_9")],
            ['Site Address',reportDetails.get("field_10")],
            ['Customer Contact Name',reportDetails.get("field_6")],
            ['Customer Contact Tel',reportDetails.get("field_7")],
            ['Customer Contact Email',reportDetails.get("field_8")],
            ['Test Conclusion',reportDetails.get("field_11")],]);


        // createTable(reportDetails.id,390,doc ,[
        //
        //
        //
        //
        //
        //
        // ]);




    }else{

        var listTables = [] ;
        //
        if(reportDetails.get("5tfzw2SRV9FS5ok36AEm")){

            dataObject["reportTitle"] = reportDetails.get("5tfzw2SRV9FS5ok36AEm");
            dataObject["reportDate"] = dateTime(reportDetails.get("created_at"));

            // createTable(reportDetails.id,230,doc ,[[ 'Report Title',reportDetails.get("5tfzw2SRV9FS5ok36AEm")],['Report Date',dateTime(reportDetails.get("created_at"))],['Compiled By',reporterName]]);
            listTables.push([ 'Report Title',reportDetails.get("5tfzw2SRV9FS5ok36AEm")],['Report Date',dateTime(reportDetails.get("created_at"))],['Compiled By',reporterName]);
        }
        // doc.text('Signature', 60,410-60, {
        //     lineBreak: false
        // });

        //  if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),110,390-60, { height: 50 , align: "center"});

        var unordered = [];

        dataObject["clientDetails"] = "";

        if(reportDetails.get("CpjUDBnKApld3QBVApEl")){
            dataObject["clientDetails"] = dataObject["clientDetails"] +reportDetails.get("CpjUDBnKApld3QBVApEl");
        }
        if(reportDetails.get("D2AKfbaYBVCSXvMO8gMS")){
            dataObject["clientDetails"] = dataObject["clientDetails"] +reportDetails.get("D2AKfbaYBVCSXvMO8gMS");

        }
        if(reportDetails.get("D8yZ0yJqnkEyDlAy1hP5")){
            dataObject["clientDetails"] = dataObject["clientDetails"] +reportDetails.get("D8yZ0yJqnkEyDlAy1hP5");

        }

        dataObject["siteDetails"] = "";


        if(reportDetails.get("Ckfetv91NIXNn4jCWtMO")){
            dataObject["siteDetails"] = dataObject["siteDetails"] +reportDetails.get("Ckfetv91NIXNn4jCWtMO");

        }
        if(reportDetails.get("syEtsFAtliHkDeGRCJAK")){
            dataObject["siteDetails"] = dataObject["siteDetails"] +reportDetails.get("syEtsFAtliHkDeGRCJAK");
        }
     dataObject["witness"] = "";
     dataObject["testConductedBy"] = "";
     dataObject["fixingType"] = "";
     dataObject["resinType"] = "";
     dataObject["baseMaterial"] = "";
     dataObject["proff"] = "";
     dataObject["holeDia"] = "";
     dataObject["edDistance"] = "";
     dataObject["holeDepth"] = "";
     dataObject["anckorCent"] = "";
     dataObject["reEmD"] = "";
     dataObject["temp"] = "";
     dataObject["curingT"] = "";
     dataObject["acTEmbDepth"] = "";
     dataObject["testEquip"] = "";
     dataObject["modelNo"] = "";
     dataObject["serialNo"] = "";

        dataObject["gaugeNo"] = "";
        dataObject["gaugeRange"] = "";
        dataObject["dateOfCalibration"] = "";
        dataObject["additional"] = "";


        if(reportDetails.get("TrFsHvS1vo1VMxiTXcdE")){
            dataObject["witness"] = dataObject["witness"] +reportDetails.get("TrFsHvS1vo1VMxiTXcdE");

        }

        if(reportDetails.get("AvBCGpka0NuEbDAzJQHr")){
            dataObject["testConductedBy"] = dataObject["testConductedBy"] +reportDetails.get("AvBCGpka0NuEbDAzJQHr");

        }

        if(reportDetails.get("4yRbKF31DwB0ggsflz9K")){
            dataObject["fixingType"] = dataObject["fixingType"] +reportDetails.get("4yRbKF31DwB0ggsflz9K");

        }
        if(reportDetails.get("9MhfOPzJCg3Iv0AvYhuV")){
            dataObject["resinType"] = dataObject["resinType"] +reportDetails.get("9MhfOPzJCg3Iv0AvYhuV");

        }

        if(reportDetails.get("LENFKMKRWtgjoV3gyY7W")){
            dataObject["baseMaterial"] = dataObject["baseMaterial"] +reportDetails.get("LENFKMKRWtgjoV3gyY7W");

        }
        if(reportDetails.get("52Iv9cIGUuvwTZdgCmE0")){
            dataObject["proff"] = dataObject["proff"] +reportDetails.get("52Iv9cIGUuvwTZdgCmE0");

        }

        if(reportDetails.get("BSxFGHg6o7kcEL3216d5")){
            dataObject["holeDia"] = dataObject["holeDia"] +reportDetails.get("BSxFGHg6o7kcEL3216d5");

        }
        if(reportDetails.get("A9i1v9cfmzx9yWDz8YC9")){
            dataObject["edDistance"] = dataObject["edDistance"] +reportDetails.get("A9i1v9cfmzx9yWDz8YC9");

        }

        if(reportDetails.get("v5guYKWsBSRylmqCctJ9")){
            dataObject["holeDepth"] = dataObject["holeDepth"] +reportDetails.get("v5guYKWsBSRylmqCctJ9");

        }
        if(reportDetails.get("PWg6RYV8h5tUymxmbFmZ")){
            dataObject["anckorCent"] = dataObject["anckorCent"] +reportDetails.get("PWg6RYV8h5tUymxmbFmZ");

        }

        if(reportDetails.get("vteURzcP2mHMQ3PpCeZE")){
            dataObject["reEmD"] = dataObject["reEmD"] +reportDetails.get("vteURzcP2mHMQ3PpCeZE");

        }

        if(reportDetails.get("81LfqzG0LDmw30wLJzNn")){
            dataObject["temp"] = dataObject["temp"] +reportDetails.get("81LfqzG0LDmw30wLJzNn");

        }
        if(reportDetails.get("fiKThRdclwShgTenosAF")){
            dataObject["curingT"] = dataObject["curingT"] +reportDetails.get("fiKThRdclwShgTenosAF");

        }

        if(reportDetails.get("Bp4qHea2MN0j1gSfSiPS")){
            dataObject["acTEmbDepth"] = dataObject["acTEmbDepth"] +reportDetails.get("Bp4qHea2MN0j1gSfSiPS");

        }
        if(reportDetails.get("isQGQ7FVSTFBOw5bqmwz")){
            dataObject["testEquip"] = dataObject["testEquip"] +reportDetails.get("isQGQ7FVSTFBOw5bqmwz");

        }

        if(reportDetails.get("k0mXw5BBVwrmrpdBezdK")){
            dataObject["modelNo"] = dataObject["modelNo"] +reportDetails.get("k0mXw5BBVwrmrpdBezdK");

        }

        if(reportDetails.get("rJZo2fmqbQ4R5rJV41lx")){
            dataObject["serialNo"] = dataObject["serialNo"] +reportDetails.get("rJZo2fmqbQ4R5rJV41lx");

        }

        if(reportDetails.get("Mt2YMH04ksdgeTkzhEJ4")){
            dataObject["gaugeNo"] = dataObject["gaugeNo"] +reportDetails.get("Mt2YMH04ksdgeTkzhEJ4");

        }

        if(reportDetails.get("6NzKsuaorGHh2bzDzbfK")){
            dataObject["gaugeRange"] = dataObject["gaugeRange"] +reportDetails.get("6NzKsuaorGHh2bzDzbfK");

        }

        if(reportDetails.get("c3o6LxGWDDHQ3vLBzUdP")){
            dataObject["dateOfCalibration"] = dataObject["dateOfCalibration"] +reportDetails.get("c3o6LxGWDDHQ3vLBzUdP");

        }
        if(reportDetails.get("iK2fqLXdKzZTUbG16WOK")){
            dataObject["additional"] = dataObject["additional"] +reportDetails.get("iK2fqLXdKzZTUbG16WOK");

        }

       // dataObject["clientDetails"] = reportDetails.get("CpjUDBnKApld3QBVApEl")+" "+reportDetails.get("D2AKfbaYBVCSXvMO8gMS")+" "+reportDetails.get("D8yZ0yJqnkEyDlAy1hP5");


        for (let [key, value] of Object.entries(reportDetails.data())) {
            console.log(`${key}: ${value}`);

            if(key == "5tfzw2SRV9FS5ok36AEm"){

            }
                // else  if(key == "CpjUDBnKApld3QBVApEl"){
                //
            // }
            else if(key == "report_by"){

            }
            else if(key == "signature"){

            }
            else  if(key == "created_at"){

            } else  if(key == "pdfFile"){

            }else{

                var rN = await  firestoreDb.collection("reportFields").doc(key).get()
                unordered.push([rN.get("name"),value,rN.get("order")]);
                // listTables.push([rN.get("name"),value]);
            }



        }
        function compare( a, b ) {
            if ( a[2] < b[2] ){
                return -1;
            }
            if ( a[2] >b[2] ){
                return 1;
            }
            return 0;
        }
        console.log("before order");
        console.log(unordered);
        unordered.sort(compare);
        console.log("after order");
        console.log(unordered);

        for(var i = 0 ; i < unordered.length ; i++){
            listTables.push([unordered[i][0],unordered[i][1]]);
        }
        if(false)   createTable(prjectName,reportDetails.id,130,doc ,listTables);

    }

   return  dataObject;
    doc.fillColor('gray').text("1", doc.page.width-30,  doc.page.height);

//.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'), 100, 300, { width: 100 });

    function compare( a, b ) {
        if ( a.get("data")["time"] < b.get("data")["time"] ){
            return 1;
        }
        if ( a.get("data")["time"] >b.get("data")["time"] ){
            return -1;
        }
        return 0;
    }
// textInRowFirst(doc, 'Customer Company Name',reportDetails.get("field_2"), 470-60);
// textInRowFirst(doc, 'Customer Address',reportDetails.get("field_5"), 500-60);
// textInRowFirst(doc, 'Site Name',reportDetails.get("field_9"), 530-60);
// textInRowFirst(doc, 'Site Address',reportDetails.get("field_10"), 560-60);
// textInRowFirst(doc, 'Customer Contact Name',reportDetails.get("field_6"), 590-60);
// textInRowFirst(doc, 'Customer Contact Tel',reportDetails.get("field_7"), 620-60);
// textInRowFirst(doc, 'Customer Contact Email',reportDetails.get("field_8"), 650-60);
// textInRowFirst(doc, 'Test Conclusion',reportDetails.get("field_11"), 680-60);

  if(false)  try{
//loop
        var allPages = await firestoreDbCustomer.collection('reports').doc(reportID).collection("items").get();
        //    console.log(allPages);








        var filterD = allPages.docs;

        var filteredAgain = [];

        for (var i = allPages.docs.length - 1; i >= 0; i--) {
            //    console.log(allPages.docs[i].get("data"));
            if(allPages.docs[i].get("data") !== null ) filteredAgain.push(allPages.docs[i]);

        }


        filteredAgain.sort( compare );

//sort all pages


        async function createPage(testItem,i1){

            var testType = 1;

            try{
                if(testItem.get("data")["type"] == "type2")testType = 2;

            }catch(e){}

            console.log(3);
            doc.addPage();
            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
            doc.fillColor('black').text('Test Title', 20, 50, {});
            doc.fillColor('gray').text(testItem.get("data")["name"], 120, 50, {});

            doc.image(new Buffer(testItem.get("data")["graphImage"].replace('data:image/png;base64,',''), 'base64'),100,70, { width: 400 , align: "center"});

//new start
            var y = 320;

            doc.fillColor("black");


            if(testType == 1){
                doc
                    .fontSize(12)
                    .text("Date & Time", 50, y)
                    .fillColor("green")
                    .text(dateTime(testItem.get("data")["time"],testType == 1?  testItem.get("data")["timeZone"]:0), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Maximum Load Achieved",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text((testItem.get("data")["max"]+" "+testItem.get("data")["loadMode"]), 370, y, { width: doc.page.width/4, align: "right" })
                doc.fillColor("black");
                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Target Load", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Device SN",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index2"], 370, y, { width: doc.page.width/4, align: "right" })

                doc.fillColor("black");
                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Timed Section Started", 50, y)
                    .fillColor("green")
                    .text(millisToMinutesAndSeconds(testItem.get("data")["startedLoad"]), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Timed Section Finished",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(millisToMinutesAndSeconds(testItem.get("data")["endedLoad"]), 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc.fillColor("black");

                doc
                    .fontSize(12)
                    .text("Timed Section Length", 50, y)
                    .fillColor("green")
                    .text(secondsToMinutesAndSeconds(testItem.get("data")["targetDuration"]), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Next Calibration Date",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index6"], 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc.fillColor("black");
                var testResult  = "Fail";

                if(testItem.get("data")["startedLoad"]  > 0){
                    if(testItem.get("data")["didPassed"]  > 0){
                        testResult  = "Pass";

                    }else{
                        testResult  = "Fail";
                    }
                }else{
                    testResult  = "Fail";
                }
                if(testItem.get("data")["startedLoad"]  == 0){
                    testResult = "Not Timed";
                }
                doc
                    .fontSize(12)
                    .text("Test Result", 50, y)
                    .fillColor("green")
                    .text(testResult, doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" });


                doc.fillColor("black");

            }else{

                doc
                    .fontSize(12)
                    .text("Date & Time", 50, y)
                    .fillColor("green")
                    .text(dateTime(testItem.get("data")["time"],0), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Maximum Load Achieved",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text((testItem.get("data")["maxForce"]+" kN"), 370, y, { width: doc.page.width/4, align: "right" })
                doc.fillColor("black");
                y = y + 25;


                doc
                    .fontSize(12)
                    .text("Device SN", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["index2"], doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Next Calibration Date",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index6"], 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Max displacement", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["maxDisplacement"]+" mm", doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")

            }




//new ends

            //     doc.fillColor('black').text('Date & Time', 50, 290, {align:"left"});
            //     doc.fillColor('green').text(dateTime(testItem.get("data")["time"]), 176, 290, {align:"left"});



            //     doc.fillColor('black').text('Target Load', 50, 320, {align:"left"});
            //    // doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 0,320, {fit: [250, 40], align: 'right', valign: 'center'});

            //    if(testItem.get("data")["loadMode"] == "kN")
            //    doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 248,320, {align:"left"});
            //    else
            //     doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 240,320, {align:"left"});

            //     doc.fillColor('black').text('Timed Section Started', 50, 350, {align:"left"});
            //     doc.fillColor('green').text(millisToMinutesAndSeconds(testItem.get("data")["startedLoad"]),  250,350, {align:"left"});




            //     doc.fillColor('black').text('Timed Section Length', 50, 380, {align:"left"});
            //     doc.fillColor('green').text(secondsToMinutesAndSeconds(testItem.get("data")["targetDuration"]), 250,380, {align:"left"});




            let nodeData = testItem.get("data")["note"];
            // nodeData = nodeData.replace(/(\r\n|\n|\r)/gm, "");
            nodeData = nodeData.replace(/(\r\n|\n|\r)/gm, " ");
            //   console.log(nodeData);
            var noteLength = nodeData.length;
            console.log(noteLength);
            doc.fillColor('black').text('Notes', 50, y+25, {align:"left"});
            doc.fillColor('green').text(nodeData, 50,y+25+25, {});
            //console.log(testItem.get("data")["note"]);



            // doc.fillColor('black').text('Maximum Load Achieved', ((doc.page.width)/2), 290, {});
            // doc.fillColor('green').text(testItem.get("data")["max"]+" "+testItem.get("data")["loadMode"], ((doc.page.width)/2), 290, {align:"right"});



            // doc.fillColor('black').text('Device SN', ((doc.page.width)/2), 320, {});
            // doc.fillColor('green').text(testItem.get("data")["index2"], ((doc.page.width)/2),320, {align:"right"});



            // doc.fillColor('black').text('Timed Section Finished', ((doc.page.width)/2), 350, {});
            // doc.fillColor('green').text(millisToMinutesAndSeconds(testItem.get("data")["endedLoad"]), ((doc.page.width)/2),350, {align:"right"});



            // doc.fillColor('black').text('Next Calibration Date', ((doc.page.width)/2), 380, {});
            // doc.fillColor('green').text(testItem.get("data")["index6"],((doc.page.width)/2),380, {align:"right"});



            var lat = testItem.get("data")["location"]["lat"];
            var long = testItem.get("data")["location"]["long"];


            var mapUri  = "https://maps.googleapis.com/maps/api/staticmap?center=" +
                lat +
                "," +
                long+
                "&zoom=8&size=500x200" +"&maptype=normal&markers=anchor:center|"+
                "icon:https%3A%2F%2Ffirebasestorage.googleapis.com%2Fv0%2Fb%2Fstaht-connect-322113.appspot.com%2Fo%2FmapMarker1Icon.png%3Falt%3Dmedia%26token%3D29e63d7e-2a40-46ec-ac38-740c4b22435d|"+
                lat +
                "," +
                long+
                "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";




            //  var mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=500x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";

            var mapImageHeight = 200;

            if(noteLength>625){
                if(noteLength<(625+90)){
                    //  mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=400x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 190;
                }else{

                }
                if(noteLength<(625+(90*2))){
                    //  mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=300x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 180;
                }


                else{

                }
                if(noteLength<(625+(90*3))){
                    // mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=200x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 170;
                }
                else{

                }
                if(noteLength<(625+(90*4))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 160;
                }else{

                }
                if(noteLength<(625+(90*5))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 150;
                }else{

                }
                if(noteLength<(625+(90*6))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 140;
                }else{

                }

                mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +
                    lat +
                    "," +
                    long+
                    "&zoom=8&size=500x200"+"&maptype=normal&markers=anchor:center|"+
                    "icon:https%3A%2F%2Ffirebasestorage.googleapis.com%2Fv0%2Fb%2Fstaht-connect-322113.appspot.com%2Fo%2FmapMarker1Icon.png%3Falt%3Dmedia%26token%3D29e63d7e-2a40-46ec-ac38-740c4b22435d|"+
                    lat +
                    "," +
                    long+
                    "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";



                console.log(mapUri);
                // mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=500x"+mapImageHeight+"&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";

            }
            doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false });
            try{
                // doc.fillColor('gray').text(mapUri, 100, 150, {});
                var im =  await axios.get(mapUri, {responseType: 'arraybuffer'})
                if(noteLength>(1165)){
                    doc.addPage();
                    doc.fillColor('black').text('Test Title', 20, 50, {});
                    doc.fillColor('gray').text(testItem.get("data")["name"], 120, 50, {});
                    doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                    doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 200)/2, { width: 500 , height : 200,align: "center"});

                }else{
                    doc.image(im.data, (doc.page.width - 500) /2, doc.page.height - (mapImageHeight+55), { width: 500 , height : mapImageHeight,align: "center"});

                }

                // doc.lineCap('butt')
                //   .moveTo(50, 20)
                //   .lineTo(100, 20)
                //   .stroke();
            }catch(err){
                console.log("Error on getting map");
                console.log(err);            }
// && testItem.get("data")["allImageGrid"]
            if(true ){
                console.log("got grid.. good");

                doc.addPage();
                doc.fillColor('black').text('Test Title', 20, 50, {});
                doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                //doc.image(im.data, (doc.page.width - 500) /2 )

                //  doc.image(im.data, {fit: [0, 0],align: 'center',valign: 'center'});
                try{
                    try{
                        var all = testItem.get("attachment");
                        console.log("got attachments");
                        console.log(all);

                        if(all.length>0){
                            console.log(" attachments number");
                            console.log(all.length);

                            var links = [];


                            for(var i = 0 ; i < all.length ; i++){
                                if(all[i]["type"] == "photo"){
                                    links.push(all[i]["link"]);
                                }

                            }
                            if(links.length>0){
                                console.log("need to marge");
                                //mm

                                try{
                                    const headers = {
                                        'Content-Type': 'application/json',

                                    }
                                    var im =  await axios.post("https://staht-connect-322113.uc.r.appspot.com/mergePhoto",JSON.stringify({"photos":links}), {
                                        headers: headers
                                    });
                                    var j =  im.data;
                                    console.log("api done");
                                    var imgDataAs64 = j["response"];
                                    console.log("response done");
                                    // console.log(im.data);



                                    doc.image(new Buffer(imgDataAs64.replace('data:image/png;base64,',''), 'base64'), (doc.page.width-466) /2, (doc.page.height-700) /2, { width: 466 , height : 700,align: "center"});

                                    console.log("img write done done");
                                }catch(ex){
                                    console.log("at axios");
                                    console.log(ex);

                                }






                            }else{
                                console.log(" no attachment here also (photo)");
                            }
                        }else{
                            console.log(" no attachment here");
                        }

                    }catch(ee3){
                        doc.text("Photo is Corrupted.Try regenerte the report", { width: 500 , height : 700,align: "center"})
                        console.log("error at seting grid photo");
                        console.log(err);

                    }
                    //var im =  await axios.get(testItem.get("data")["allImageGrid"], {responseType: 'arraybuffer'});
                    // doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 700) /2, { width: 500 , height : 700,align: "center"});

                }catch(err){











                }





                //  doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                // doc.image(im.data, (doc.page.width - 500) /2, doc.page.height - 700, { width: 500 , height : mapImageHeight,align: "center"});


            }else{
                console.log("No Grid.making photo..bad bad bad");
                var link = "https://us-central1-staht-connect-322113.cloudfunctions.net/mergePhotosAndSave";
                //https://staht-connect-322113.uc.r.appspot.com/mergePhoto


                var photoList = [];
                var xM =  ((doc.page.width)-30)/2 ;
                var yM = ((doc.page.height)-160)/3 ;
                if(isTest){

                    // xM =(doc.page.width-100)/2;
                    //  yM =(doc.page.height-130)/3;
                    //  doc.rotate(90, {origin : [0, 0]});
                }
                const options = {
                    quality: 40,
                    jpegjsMaxResolutionInMP: 1234,
                }
                //testItem.get("attachment")
                if(testItem.get("attachment")){
                    if ( testItem.get("attachment").length>0) {




                        for (var i = 0; i < testItem.get("attachment").length ; i++) {
                            if(testItem.get("attachment")[i]["type"] == "photo") photoList.push(testItem.get("attachment")[i]["link"]);
                        }
                        if(photoList.length >0){
                            doc.addPage();

                            doc.fillColor('black').text('Test Title', 20, 50, {});
                            doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                            doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                            var response =  await axios.post(link, { photoList: photoList,test_id:testItem.id });
                            console.log(response.data);

                            var im =  await axios.get(response.data["status"], {responseType: 'arraybuffer'});
                            //doc.image(im.data, (doc.page.width - 500) /2 )

                            //  doc.image(im.data, {fit: [0, 0],align: 'center',valign: 'center'});
                            doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 700) /2, { width: 500 , height : 700,align: "center"});
                            //  console.log("test id "+testItem.id);
                            // console.log("grid here");
                            // console.log(response.data);
                            // firestoreDbCustomer.collection('reports').doc(reportID).collection("items").doc();
                            // testItem.reference.updateDoc({"test":"test"});
                            //  updateDoc( testItem.reference, {
                            //      "test":"test"
                            //  });

                        }
                        if(false &&   photoList.length >0)
                            for (var i = 0 ; i <photoList.length; i++) {
                                console.log("img ");
                                console.log(i);
                                if(i == 0){
                                    // doc.save();
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'});


                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            //console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log("this error ");
                                            console.log(err);
                                        }

                                        // var rotatedImg = await thenable;

                                        // doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});




                                        //  doc.image(im.data, 50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 1){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [0, 0]});
                                    if(isTest){
                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*1 +(20), yM*0+(80),  {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        //  doc.image(im.data, 50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1 +(20), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 2){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            //console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        // doc.image(rotatedImg,   xM*0+(10), yM*1+(90),  {fit: [xM,yM], align: 'center', valign: 'center'});


                                        // doc.image(im.data,xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 3){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+50,-yM-yM-60, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 4){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //   doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 5){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }


                                        //  doc.image(im.data,xM+xM+50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 6){
                                    doc.addPage();
                                    doc.fillColor('black').text('Test Title', 20, 50, {});
                                    doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                                    doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'});


                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            //console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log("this error ");
                                            console.log(err);
                                        }

                                        // var rotatedImg = await thenable;

                                        // doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});




                                        //  doc.image(im.data, 50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 7){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [0, 0]});
                                    if(isTest){
                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*1 +(20), yM*0+(80),  {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        //  doc.image(im.data, 50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1 +(20), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 8){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            //console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        // doc.image(rotatedImg,   xM*0+(10), yM*1+(90),  {fit: [xM,yM], align: 'center', valign: 'center'});


                                        // doc.image(im.data,xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 9){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+50,-yM-yM-60, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 10){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //   doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 11){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }


                                        //  doc.image(im.data,xM+xM+50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }
                                // doc.restore();
                            }

                    }
                }

            }










// for (var j = testItem.get("attachment").length - 1; j >= 0; j--) {
//     var photoCount = 0 ;
//     if(testItem.get("attachment")[j]["type"] == "photo"){
//         photoCount ++;

//           //   doc.fillColor('gray').text((String(i1)), doc.page.width-30,  doc.page.height);







//     if(photoCount == 1){
//         // var im =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//        //  doc.image(im.data, 50, 70, { width: 146 , height : 260,align: "center"});
//         // doc.image(im.data, 200, 70, { width: 146 , height : 260,align: "center"});

//       }

//     // if(photoCount == 2){
//     //      var im2 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im2.data, 200, 70, { width: 146 , height : 260,align: "center"});
//     //   }


//     // if(photoCount == 3){
//     //      var im3 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im3.data, 50, 360, { width: 200 , height : 260,align: "center"});
//     //   }
//     // if(photoCount == 4){
//     //      var im4 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im4.data, 300, 360, { width: 200 , height : 260,align: "center"});
//     //   }

//     // if(photoCount == 5){
//     //      var im5 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im5.data, 50, 420, { width: 200 , height : 260,align: "center"});
//     //   }
//     // if(photoCount == 6){
//     //      var im6 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im6.data, 300, 420, { width: 200 , height : 260,align: "center"});
//     //   }
//       }

// }
//if(testItem.get("grid")) doc.image(new Buffer(testItem.get("grid").replace('data:image/png;base64,',''), 'base64'), (doc.page.width - 500) /2, 80, { width: 500 ,align: "center"});
//if(testItem.get("grid")) doc.image(new Buffer(testItem.get("grid").replace('data:image/png;base64,',''), 'base64'),20,70, { width: 500 , align: "center"});
        }
        for (var i = filteredAgain.length - 1; i >= 0; i--) {
            await createPage(filteredAgain[i], filteredAgain.length-(2*i));
        }

    }catch(err){
        console.log("Could not get Test items");
        console.log(err);
        // res.send("Could not get Test items");
    }




}
async function generateHeader(doc,customerInfo,reportID,isTest) {
    function millisToMinutesAndSeconds(millis) {
        var minutes = Math.floor(millis / 60000);
        var seconds = ((millis % 60000) / 1000).toFixed(0);
        return minutes + ":" + (seconds < 10 ? '0' : '') + seconds;
    }
    function secondsToMinutesAndSeconds(millis) {
        var minutes = Math.floor(millis / 60);
        var seconds = ((millis % 60000) / 1).toFixed(0);
        return minutes + ":" + (seconds < 10 ? '0' : '') + seconds;
    }
    function createTable(mainCompanyName,testTitle,startingY,doc, data, width = doc.page.width-100) {
        const startY = startingY;
        const  startX = 50;
        let distanceY = 20;
        let distanceX = 10;

        doc.fillColor("grey");
        doc.fontSize(12);
        doc.lineWidth(0.005);
        let currentY = startY;

        var heightYY = 15 ;

        data.forEach(value => {

            // var dataValue =  value[1].replace(/(\r\n|\n|\r)/gm, "");
            var dataValue =  value[1].replace(/\n\s*\n/g, '\n');

            dataValue = dataValue.replace(/\n\s*\n\s*\n/g, '\n');

            var breakCount = dataValue.split(/\r\n|\r|\n/).length;
            if(dataValue.length<60)distanceY = 25;
            else  if(dataValue.length<120)distanceY = heightYY*2+(10);
            else  if(dataValue.length<180)distanceY = heightYY*3+(10);
            else  if(dataValue.length<240)distanceY = heightYY*4+(10);
            else  if(dataValue.length<300)distanceY = heightYY*5+(10);
            else  if(dataValue.length<360)distanceY = heightYY*6+(10);
            else  if(dataValue.length<420)distanceY = heightYY*7+(10);
            else  if(dataValue.length<480)distanceY = heightYY*8+(10);
            else  if(dataValue.length<520)distanceY = heightYY*9+(10);
            else  if(dataValue.length<(520+(60*1)))distanceY = heightYY*10+(10);
            else  if(dataValue.length<(520+(60*2)))distanceY = heightYY*11+(10);
            else  if(dataValue.length<(520+(60*3)))distanceY = heightYY*12+(10);
            else  if(dataValue.length<(520+(60*4)))distanceY = heightYY*13+(10);
            else  if(dataValue.length<(520+(60*5)))distanceY = heightYY*14+(10);
            else  if(dataValue.length<(520+(60*6)))distanceY = heightYY*15+(10);


            else distanceY = heightYY*16+(10);

            if(breakCount>1){
                distanceY = distanceY + breakCount*heightYY;
            }else if(value[0].length > 26){
                distanceY = distanceY + 1*heightYY;

                //distanceY = heightYY*2+(10);
                //currentY = currentY +heightYY ;
            }

            distanceX = 10;
            let currentX = startX,
                size = value.length;
            let nowprinting = 0;

            let blockSize = width / size;

            value.forEach(text => {
                if(nowprinting == 0){
                    blockSize = width*0.33;
                    nowprinting ++ ;
                }else{
                    text = dataValue;
                    blockSize = width*0.67;
                    nowprinting ++ ;
                }
                //Write text


                if(currentY>750 ||((currentY + distanceY) > 750) ){

                    currentY = 80;
                    doc.addPage();
                    doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
                    doc.fontSize(12);
                    doc.fillColor('grey');
                    doc
                        .lineWidth(0.005);

                    doc.strokeColor("grey");


                    //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
                    //    doc.fillColor('black').text('Report link', 20, 50, {});
                    doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});
                    doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                        lineBreak: false
                    });
                }


                if(nowprinting == 1){





                    doc.font(fontBold.data);
                    doc.fillColor("black");

                    var oldText = text;
                    if(true || oldText.length>25){
                        doc.text(oldText,currentX + distanceX, currentY+8, { width: blockSize*0.95 , align: "left"});


                        // var a = oldText;
                        // var b = "\n";
                        // var position = 25;
                        // var output = [a.slice(0, position), b, a.slice(position)].join('');
                        //
                        // doc.text(output, currentX + distanceX, currentY+8);
                    }else
                        doc.text(text, currentX + distanceX, currentY+8);
                }else{
                    doc.font(fontRegular.data);
                    doc.fillColor("#000000");
                    doc.text(text, currentX + distanceX, currentY+8);






                }




                //   doc.fillColor("red");
                doc.strokeColor("grey");
                //Create rectangles
                doc
                    .lineJoin("miter")
                    .rect(currentX, currentY, blockSize, distanceY)
                    .stroke()
                    .lineWidth(0.005)
                    .strokeColor("grey");

                currentX += blockSize;


            });

          //  currentY += distanceY;



        });



        if(currentY>750){

            currentY = 80;
            doc.addPage();
            doc
                .lineWidth(0.005);

            doc.strokeColor("grey");
            doc.fillColor('black').text(mainCompanyName, 50,  50, { height: 50 , align: "center"}).fontSize(25);
            doc.fontSize(12);
            doc.fillColor('grey');

           if(false) doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

           // if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});




            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
            //  doc.fillColor('black').text('Report link', 20, 50, {});

            // doc.text('Signature', 60,currentY, {
            //     lineBreak: false
            // });

            //   if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),110,currentY, { height: 50 , align: "center"});
            doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
                lineBreak: false
            });
            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});


        }else{
            currentY =   currentY+50 ;
       if(false)     doc.text('Signature', 60,currentY, {
                lineBreak: false
            });

          //  if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),150,currentY, { height: 50 , align: "center"});


            doc.fillColor('gray').text("https://app.staht.com/#/"+testTitle, 80, doc.page.height - 50, { height: 50 , align: "right"});

        }


    }
    function dateTime(argument,zone) {
        var time= moment.utc(argument ).local().format('DD MMM YYYY HH:mm');

        if(zone){
            time = moment(argument).zone(zone).format('DD MMM YYYY HH:mm');
        }else{
            // var summerTimeEnds = 1635645600;

            // if(argument>summerTimeEnds){
            //      time = moment(argument).zone(60).format('DD MMM YYYY HH:mm');
            // }else{
            //     time = moment(argument).zone(-60).format('DD MMM YYYY HH:mm');
            // }


        }

        // var time = moment(argument).zone(0).format('DD MMM YYYY HH:mm');
        return time;

// var options = {year: 'numeric', month: 'long', day: '2-digit', hour: '2-digit', minute: '2-digit', };
// var d = new Date(argument);
// var ds = d.toLocaleDateString("en-US", options);
// return ds;
    }
    function textInRowFirst(doc, text,val, heigth) {
        doc.y = heigth+10;
        doc.x = 30;
        doc.fillColor('black')
        doc.text(text, {
            paragraphGap: 5,
            indent: 5,
            align: 'left',
            columns: 1,
        });
        doc.y = heigth+10;
        doc.x = 200;
        doc.fillColor('black')
        doc.text(val, {
            paragraphGap: 5,
            indent: 5,
            align: 'right',
            columns: 1,
        });
        return doc
    }


    function row(doc, heigth) {
        doc.lineJoin('miter')
            .rect(30, heigth, doc.page.width-70, 30)
            .stroke()
        return doc
    }
    function rowBig(doc, heigth,level) {
        doc.lineJoin('miter')
            .rect(30, heigth, doc.page.width-60, 12+(18*(level)))
            .stroke()
        return doc
    }
//doc.image((new Buffer(image.replace('data:image/png;base64,',orgLogoBase64), 'base64'), 100, 100));

// Pipe its output somewhere, like to a file or HTTP response
// See below for browser usage
//doc.pipe(fs.createWriteStream('output.pdf'));

// Embed a font, set the font size, and render some text

    const firestoreDb = admin.firestore();



    //seperate db
    console.log(customerInfo.get("projectId"));


    try{




        admin.initializeApp({
            //credential: admin.credential.cert(serviceAccount)
            credential: admin.credential.cert({
                privateKey : superKey,
                clientEmail : superEmail,
                // privateKey: "-----BEGIN PRIVATE KEY-----\\ns9o2ZsCf89CTRWexlVA9ibon\n-----END PRIVATE KEY-----\n",
                //  clientEmail:  "allpermissions@mindful-vial-304611.iam.gserviceaccount.com",
                projectId: customerInfo.get("projectId"),
            })


        }, customerInfo.get("projectId"));
    }catch (err) {
        console.log("admin initialize error");

        //res.send("Could not open Database");
    }
    const firestoreDbCustomer= admin.app( customerInfo.get("projectId")).firestore();

    //seperate db end


    var conclutionLevel = 1 ;

    const reportDetails = await firestoreDbCustomer.collection('reports').doc(reportID).get();
    console.log(1);
//reportDetails.get("field_11")
    if(reportDetails.get("field_11")){
        if(reportDetails.get("field_11").length > 60)  conclutionLevel = 2;
        if(reportDetails.get("field_11").length > 120)  conclutionLevel = 3;
        if(reportDetails.get("field_11").length > 120)  conclutionLevel = 4;
        if(reportDetails.get("field_11").length > 180)  conclutionLevel = 5;
        if(reportDetails.get("field_11").length > 240)  conclutionLevel = 6;
        if(reportDetails.get("field_11").length > 300)  conclutionLevel = 7;
        if(reportDetails.get("field_11").length > 360)  conclutionLevel = 8;
        if(reportDetails.get("field_11").length > 420)  conclutionLevel = 9;

    }else{
        conclutionLevel =1;
    }
    var reporterName = "Empty";


    if(firestoreDb){
        console.log(2);


        var cs = await firestoreDb.collection("users").where("uid","==",reportDetails.get("report_by")).get();


        for (var i = cs.docs.length - 1; i >= 0; i--) {
            reporterName = cs.docs[i].get("displayName");
        }



//   const reporter = await firestoreDb.collection('users').where("uid","==",reportDetails.get("report_by")).get();

// reporter.forEach((doc) => {
//   // doc.data() is never undefined for query doc snapshots
//   console.log(doc.id, " => ", doc.data());
//   //reporterName = doc.get("displayName");
// });
    }


    var  logo =  customerInfo.get("logo");
    var  address =  customerInfo.get("address").toUpperCase();
    var  telephone =  customerInfo.get("telephone");
    var  prjectName =  customerInfo.get("prjectName").toUpperCase();
    var  email =  customerInfo.get("email");
    doc.moveDown();
    doc.fillColor("gray").text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {
        lineBreak: false
    });
    // .image(new Buffer(logo.replace('data:image/png;base64,',''), 'base64'),doc.page.width-130,10,  {fit: [130, 130], align: 'right', valign: 'center'})
    if(customerInfo.get("logo")){
        doc
            .image(new Buffer(logo.replace('data:image/png;base64,',''), 'base64'),doc.page.width-130-50, 10,{fit: [130, 130], align: 'right', valign: 'bottom'})
            .fillColor("#000000")
            .fontSize(15)
            .text(prjectName, 0, 150,{ align: "right" })
            .fontSize(12)
            .fillColor("#000000")
            .text(address, 0, 170, { align: "right" })
            .fontSize(12)
            .text(email, 0, 192, { align: "right" })
            .fontSize(12)
            .text(telephone, 0, 210, { align: "right" })
            .moveDown();
    }else{
        doc
            // .image(new Buffer("wwww", 'base64'),doc.page.width-130-50, 10,{fit: [130, 130], align: 'right', valign: 'bottom'})
            .fillColor("#000000")
            .fontSize(15)
            .text(prjectName, 0, 150,{ align: "right" })
            .fontSize(12)
            .fillColor("#000000")
            .text(address, 0, 175, { align: "right" })
            .fontSize(12)
            .text(email, 0, 192, { align: "right" })
            .fontSize(12)
            .text(telephone, 0, 210, { align: "right" })
            .moveDown();
    }


// doc.lineCap('butt')
//   .moveTo(200, 310-60)
//   .lineTo(200, 400-60)
//   .stroke()

    // doc.lineCap('butt')
    // .moveTo(200, 470-60)
    // .lineTo(200, 710-60+(conclutionLevel-1)*18)
    // .stroke()


// row(doc, 310-60);
// row(doc, 340-60);
// row(doc, 370-60);


// row(doc, 470-60);

// row(doc, 500-60);
// row(doc, 530-60);
// row(doc, 560-60);
// row(doc, 590-60);
// row(doc, 620-60);
// row(doc, 650-60);
// rowBig(doc, 680-60,conclutionLevel);
    doc
        .lineWidth(0.005);

    doc.strokeColor("grey");
    doc.fontSize(12);

// textInRowFirst(doc, 'Report Title',reportDetails.get("field_1"), 310-60);
// textInRowFirst(doc, 'Report Date',dateTime(reportDetails.get("created_at")), 340-60);
// textInRowFirst(doc, 'Compiled By',reporterName, 370-60);




    if(reportDetails.get("field_1")){

        createTable(prjectName,reportDetails.id,230,doc ,[
            [ 'Report Title',reportDetails.get("field_1")],
            ['Report Date',dateTime(reportDetails.get("created_at"))],
            ['Compiled By',reporterName],
            [ 'Customer Company Name',reportDetails.get("field_2")],
            ['Customer Address',reportDetails.get("field_5")],
            ['Site Name',reportDetails.get("field_9")],
            ['Site Address',reportDetails.get("field_10")],
            ['Customer Contact Name',reportDetails.get("field_6")],
            ['Customer Contact Tel',reportDetails.get("field_7")],
            ['Customer Contact Email',reportDetails.get("field_8")],
            ['Test Conclusion',reportDetails.get("field_11")],]);


        // createTable(reportDetails.id,390,doc ,[
        //
        //
        //
        //
        //
        //
        // ]);




    }else{

        var listTables = [] ;
        //
        if(reportDetails.get("5tfzw2SRV9FS5ok36AEm")){
            // createTable(reportDetails.id,230,doc ,[[ 'Report Title',reportDetails.get("5tfzw2SRV9FS5ok36AEm")],['Report Date',dateTime(reportDetails.get("created_at"))],['Compiled By',reporterName]]);
            listTables.push([ 'Report Title',reportDetails.get("5tfzw2SRV9FS5ok36AEm")],['Report Date',dateTime(reportDetails.get("created_at"))],['Compiled By',reporterName]);
        }
        // doc.text('Signature', 60,410-60, {
        //     lineBreak: false
        // });

        //  if(reportDetails.get("signature")) doc.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'),110,390-60, { height: 50 , align: "center"});

       var unordered = [];
        for (let [key, value] of Object.entries(reportDetails.data())) {
            console.log(`${key}: ${value}`);

            if(key == "5tfzw2SRV9FS5ok36AEm"){

            }
                // else  if(key == "CpjUDBnKApld3QBVApEl"){
                //
            // }
            else if(key == "report_by"){

            }
            else if(key == "signature"){

            }
            else  if(key == "created_at"){

            } else  if(key == "pdfFile"){

            }else{

                var rN = await  firestoreDb.collection("reportFields").doc(key).get()
                unordered.push([rN.get("name"),value,rN.get("order")]);
               // listTables.push([rN.get("name"),value]);
            }



        }
        function compare( a, b ) {
            if ( a[2] < b[2] ){
                return -1;
            }
            if ( a[2] >b[2] ){
                return 1;
            }
            return 0;
        }
        console.log("before order");
        console.log(unordered);
        unordered.sort(compare);
        console.log("after order");
        console.log(unordered);

        for(var i = 0 ; i < unordered.length ; i++){
            listTables.push([unordered[i][0],unordered[i][1]]);
        }
        createTable(prjectName,reportDetails.id,230,doc ,listTables);

    }


    doc.fillColor('gray').text("1", doc.page.width-30,  doc.page.height);

//.image(new Buffer(reportDetails.get("signature").replace('data:image/png;base64,',''), 'base64'), 100, 300, { width: 100 });

    function compare( a, b ) {
        if ( a.get("data")["time"] < b.get("data")["time"] ){
            return 1;
        }
        if ( a.get("data")["time"] >b.get("data")["time"] ){
            return -1;
        }
        return 0;
    }
// textInRowFirst(doc, 'Customer Company Name',reportDetails.get("field_2"), 470-60);
// textInRowFirst(doc, 'Customer Address',reportDetails.get("field_5"), 500-60);
// textInRowFirst(doc, 'Site Name',reportDetails.get("field_9"), 530-60);
// textInRowFirst(doc, 'Site Address',reportDetails.get("field_10"), 560-60);
// textInRowFirst(doc, 'Customer Contact Name',reportDetails.get("field_6"), 590-60);
// textInRowFirst(doc, 'Customer Contact Tel',reportDetails.get("field_7"), 620-60);
// textInRowFirst(doc, 'Customer Contact Email',reportDetails.get("field_8"), 650-60);
// textInRowFirst(doc, 'Test Conclusion',reportDetails.get("field_11"), 680-60);

    try{
//loop
        var allPages = await firestoreDbCustomer.collection('reports').doc(reportID).collection("items").get();
        //    console.log(allPages);








        var filterD = allPages.docs;

        var filteredAgain = [];

        for (var i = allPages.docs.length - 1; i >= 0; i--) {
            //    console.log(allPages.docs[i].get("data"));
            if(allPages.docs[i].get("data") !== null ) filteredAgain.push(allPages.docs[i]);

        }


        filteredAgain.sort( compare );

//sort all pages


        async function createPage(testItem,i1){

            var testType = 1;

            try{
                if(testItem.get("data")["type"] == "type2")testType = 2;

            }catch(e){}

            console.log(3);
            doc.addPage();
            //doc.fillColor('gray').text((String(i1+1)), doc.page.width-30,  doc.page.height);
            doc.fillColor('black').text('Test Title', 20, 50, {});
            doc.fillColor('gray').text(testItem.get("data")["name"], 120, 50, {});

            doc.image(new Buffer(testItem.get("data")["graphImage"].replace('data:image/png;base64,',''), 'base64'),100,70, { width: 400 , align: "center"});

//new start
            var y = 320;

            doc.fillColor("black");


            if(testType == 1){
                doc
                    .fontSize(12)
                    .text("Date & Time", 50, y)
                    .fillColor("green")
                    .text(dateTime(testItem.get("data")["time"],testType == 1?  testItem.get("data")["timeZone"]:0), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Maximum Load Achieved",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text((testItem.get("data")["max"]+" "+testItem.get("data")["loadMode"]), 370, y, { width: doc.page.width/4, align: "right" })
                doc.fillColor("black");
                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Target Load", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Device SN",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index2"], 370, y, { width: doc.page.width/4, align: "right" })

                doc.fillColor("black");
                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Timed Section Started", 50, y)
                    .fillColor("green")
                    .text(millisToMinutesAndSeconds(testItem.get("data")["startedLoad"]), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Timed Section Finished",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(millisToMinutesAndSeconds(testItem.get("data")["endedLoad"]), 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc.fillColor("black");

                doc
                    .fontSize(12)
                    .text("Timed Section Length", 50, y)
                    .fillColor("green")
                    .text(secondsToMinutesAndSeconds(testItem.get("data")["targetDuration"]), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Next Calibration Date",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index6"], 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc.fillColor("black");
                var testResult  = "Fail";

                if(testItem.get("data")["startedLoad"]  > 0){
                    if(testItem.get("data")["didPassed"]  > 0){
                        testResult  = "Pass";

                    }else{
                        testResult  = "Fail";
                    }
                }else{
                    testResult  = "Fail";
                }
                if(testItem.get("data")["startedLoad"]  == 0){
                    testResult = "Not Timed";
                }
                doc
                    .fontSize(12)
                    .text("Test Result", 50, y)
                    .fillColor("green")
                    .text(testResult, doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" });


                doc.fillColor("black");

            }else{

                doc
                    .fontSize(12)
                    .text("Date & Time", 50, y)
                    .fillColor("green")
                    .text(dateTime(testItem.get("data")["time"],0), doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Maximum Load Achieved",  8+doc.page.width/2, y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text((testItem.get("data")["maxForce"]+" kN"), 370, y, { width: doc.page.width/4, align: "right" })
                doc.fillColor("black");
                y = y + 25;


                doc
                    .fontSize(12)
                    .text("Device SN", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["index2"], doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")
                    .text("Next Calibration Date",  8+doc.page.width/2,y, { width: doc.page.width/4, align: "left" })
                    .fillColor("green")
                    .text(testItem.get("data")["index6"], 370, y, { width: doc.page.width/4, align: "right" })

                y = y + 25;
                doc
                    .fontSize(12)
                    .text("Max displacement", 50, y)
                    .fillColor("green")
                    .text(testItem.get("data")["maxDisplacement"]+" mm", doc.page.width/4, y,{ width: -8+doc.page.width/4, align: "right" })
                    .fillColor("black")

            }




//new ends

            //     doc.fillColor('black').text('Date & Time', 50, 290, {align:"left"});
            //     doc.fillColor('green').text(dateTime(testItem.get("data")["time"]), 176, 290, {align:"left"});



            //     doc.fillColor('black').text('Target Load', 50, 320, {align:"left"});
            //    // doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 0,320, {fit: [250, 40], align: 'right', valign: 'center'});

            //    if(testItem.get("data")["loadMode"] == "kN")
            //    doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 248,320, {align:"left"});
            //    else
            //     doc.fillColor('green').text(testItem.get("data")["targetLoad"]+" "+testItem.get("data")["loadMode"], 240,320, {align:"left"});

            //     doc.fillColor('black').text('Timed Section Started', 50, 350, {align:"left"});
            //     doc.fillColor('green').text(millisToMinutesAndSeconds(testItem.get("data")["startedLoad"]),  250,350, {align:"left"});




            //     doc.fillColor('black').text('Timed Section Length', 50, 380, {align:"left"});
            //     doc.fillColor('green').text(secondsToMinutesAndSeconds(testItem.get("data")["targetDuration"]), 250,380, {align:"left"});




            let nodeData = testItem.get("data")["note"];
            // nodeData = nodeData.replace(/(\r\n|\n|\r)/gm, "");
            nodeData = nodeData.replace(/(\r\n|\n|\r)/gm, " ");
            //   console.log(nodeData);
            var noteLength = nodeData.length;
            console.log(noteLength);
            doc.fillColor('black').text('Notes', 50, y+25, {align:"left"});
            doc.fillColor('green').text(nodeData, 50,y+25+25, {});
            //console.log(testItem.get("data")["note"]);



            // doc.fillColor('black').text('Maximum Load Achieved', ((doc.page.width)/2), 290, {});
            // doc.fillColor('green').text(testItem.get("data")["max"]+" "+testItem.get("data")["loadMode"], ((doc.page.width)/2), 290, {align:"right"});



            // doc.fillColor('black').text('Device SN', ((doc.page.width)/2), 320, {});
            // doc.fillColor('green').text(testItem.get("data")["index2"], ((doc.page.width)/2),320, {align:"right"});



            // doc.fillColor('black').text('Timed Section Finished', ((doc.page.width)/2), 350, {});
            // doc.fillColor('green').text(millisToMinutesAndSeconds(testItem.get("data")["endedLoad"]), ((doc.page.width)/2),350, {align:"right"});



            // doc.fillColor('black').text('Next Calibration Date', ((doc.page.width)/2), 380, {});
            // doc.fillColor('green').text(testItem.get("data")["index6"],((doc.page.width)/2),380, {align:"right"});



            var lat = testItem.get("data")["location"]["lat"];
            var long = testItem.get("data")["location"]["long"];


            var mapUri  = "https://maps.googleapis.com/maps/api/staticmap?center=" +
                lat +
                "," +
                long+
                "&zoom=8&size=500x200" +"&maptype=normal&markers=anchor:center|"+
                "icon:https%3A%2F%2Ffirebasestorage.googleapis.com%2Fv0%2Fb%2Fstaht-connect-322113.appspot.com%2Fo%2FmapMarker1Icon.png%3Falt%3Dmedia%26token%3D29e63d7e-2a40-46ec-ac38-740c4b22435d|"+
                lat +
                "," +
                long+
                "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";




            //  var mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=500x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";

            var mapImageHeight = 200;

            if(noteLength>625){
                if(noteLength<(625+90)){
                    //  mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=400x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 190;
                }else{

                }
                if(noteLength<(625+(90*2))){
                    //  mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=300x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 180;
                }


                else{

                }
                if(noteLength<(625+(90*3))){
                    // mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=200x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 170;
                }
                else{

                }
                if(noteLength<(625+(90*4))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 160;
                }else{

                }
                if(noteLength<(625+(90*5))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 150;
                }else{

                }
                if(noteLength<(625+(90*6))){
                    //   mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=100x200&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";
                    mapImageHeight = 140;
                }else{

                }

                mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +
                    lat +
                    "," +
                    long+
                    "&zoom=8&size=500x200"+"&maptype=normal&markers=anchor:center|"+
                    "icon:https%3A%2F%2Ffirebasestorage.googleapis.com%2Fv0%2Fb%2Fstaht-connect-322113.appspot.com%2Fo%2FmapMarker1Icon.png%3Falt%3Dmedia%26token%3D29e63d7e-2a40-46ec-ac38-740c4b22435d|"+
                    lat +
                    "," +
                    long+
                    "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";



                console.log(mapUri);
                // mapUri = "https://maps.googleapis.com/maps/api/staticmap?center=" +lat + "," + long+ "&zoom=14&size=500x"+mapImageHeight+"&markers=" + lat+"," + long+ "&key=AIzaSyDOrDoiG0aLER7Y8cv2QhNr182Z0H5pMVw";

            }
            doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false });
            try{
                // doc.fillColor('gray').text(mapUri, 100, 150, {});
                var im =  await axios.get(mapUri, {responseType: 'arraybuffer'})
                if(noteLength>(1165)){
                    doc.addPage();
                    doc.fillColor('black').text('Test Title', 20, 50, {});
                    doc.fillColor('gray').text(testItem.get("data")["name"], 120, 50, {});
                    doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                    doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 200)/2, { width: 500 , height : 200,align: "center"});

                }else{
                    doc.image(im.data, (doc.page.width - 500) /2, doc.page.height - (mapImageHeight+55), { width: 500 , height : mapImageHeight,align: "center"});

                }

                // doc.lineCap('butt')
                //   .moveTo(50, 20)
                //   .lineTo(100, 20)
                //   .stroke();
            }catch(err){
                console.log("Error on getting map");
                console.log(err);            }
// && testItem.get("data")["allImageGrid"]
            if(true ){
                console.log("got grid.. good");

                doc.addPage();
                doc.fillColor('black').text('Test Title', 20, 50, {});
                doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                //doc.image(im.data, (doc.page.width - 500) /2 )

                //  doc.image(im.data, {fit: [0, 0],align: 'center',valign: 'center'});
                try{
                    try{
                        var all = testItem.get("attachment");
                        console.log("got attachments");
                        console.log(all);

                        if(all.length>0){
                            console.log(" attachments number");
                            console.log(all.length);

                            var links = [];


                            for(var i = 0 ; i < all.length ; i++){
                                if(all[i]["type"] == "photo"){
                                    links.push(all[i]["link"]);
                                }

                            }
                            if(links.length>0){
                                console.log("need to marge");
                                //mm

                                try{
                                    const headers = {
                                        'Content-Type': 'application/json',

                                    }
                                    var im =  await axios.post("https://staht-connect-322113.uc.r.appspot.com/mergePhoto",JSON.stringify({"photos":links}), {
                                        headers: headers
                                    });
                                    var j =  im.data;
                                    console.log("api done");
                                    var imgDataAs64 = j["response"];
                                    console.log("response done");
                                   // console.log(im.data);



                                    doc.image(new Buffer(imgDataAs64.replace('data:image/png;base64,',''), 'base64'), (doc.page.width-466) /2, (doc.page.height-700) /2, { width: 466 , height : 700,align: "center"});

                                    console.log("img write done done");
                                }catch(ex){
                                    console.log("at axios");
                                    console.log(ex);

                                }






                            }else{
                                console.log(" no attachment here also (photo)");
                            }
                        }else{
                            console.log(" no attachment here");
                        }

                    }catch(ee3){
                        doc.text("Photo is Corrupted.Try regenerte the report", { width: 500 , height : 700,align: "center"})
                        console.log("error at seting grid photo");
                        console.log(err);

                    }
                    //var im =  await axios.get(testItem.get("data")["allImageGrid"], {responseType: 'arraybuffer'});
                    // doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 700) /2, { width: 500 , height : 700,align: "center"});

                }catch(err){











                }





                //  doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                // doc.image(im.data, (doc.page.width - 500) /2, doc.page.height - 700, { width: 500 , height : mapImageHeight,align: "center"});


            }else{
                console.log("No Grid.making photo..bad bad bad");
                var link = "https://us-central1-staht-connect-322113.cloudfunctions.net/mergePhotosAndSave";
                //https://staht-connect-322113.uc.r.appspot.com/mergePhoto


                var photoList = [];
                var xM =  ((doc.page.width)-30)/2 ;
                var yM = ((doc.page.height)-160)/3 ;
                if(isTest){

                    // xM =(doc.page.width-100)/2;
                    //  yM =(doc.page.height-130)/3;
                    //  doc.rotate(90, {origin : [0, 0]});
                }
                const options = {
                    quality: 40,
                    jpegjsMaxResolutionInMP: 1234,
                }
                //testItem.get("attachment")
                if(testItem.get("attachment")){
                    if ( testItem.get("attachment").length>0) {




                        for (var i = 0; i < testItem.get("attachment").length ; i++) {
                            if(testItem.get("attachment")[i]["type"] == "photo") photoList.push(testItem.get("attachment")[i]["link"]);
                        }
                        if(photoList.length >0){
                            doc.addPage();

                            doc.fillColor('black').text('Test Title', 20, 50, {});
                            doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                            doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                            var response =  await axios.post(link, { photoList: photoList,test_id:testItem.id });
                            console.log(response.data);

                            var im =  await axios.get(response.data["status"], {responseType: 'arraybuffer'});
                            //doc.image(im.data, (doc.page.width - 500) /2 )

                            //  doc.image(im.data, {fit: [0, 0],align: 'center',valign: 'center'});
                            doc.image(im.data, (doc.page.width - 500) /2, (doc.page.height - 700) /2, { width: 500 , height : 700,align: "center"});
                            //  console.log("test id "+testItem.id);
                            // console.log("grid here");
                            // console.log(response.data);
                            // firestoreDbCustomer.collection('reports').doc(reportID).collection("items").doc();
                            // testItem.reference.updateDoc({"test":"test"});
                            //  updateDoc( testItem.reference, {
                            //      "test":"test"
                            //  });

                        }
                        if(false &&   photoList.length >0)
                            for (var i = 0 ; i <photoList.length; i++) {
                                console.log("img ");
                                console.log(i);
                                if(i == 0){
                                    // doc.save();
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'});


                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            //console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log("this error ");
                                            console.log(err);
                                        }

                                        // var rotatedImg = await thenable;

                                        // doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});




                                        //  doc.image(im.data, 50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 1){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [0, 0]});
                                    if(isTest){
                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*1 +(20), yM*0+(80),  {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        //  doc.image(im.data, 50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1 +(20), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 2){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            //console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        // doc.image(rotatedImg,   xM*0+(10), yM*1+(90),  {fit: [xM,yM], align: 'center', valign: 'center'});


                                        // doc.image(im.data,xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 3){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+50,-yM-yM-60, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 4){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //   doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 5){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }


                                        //  doc.image(im.data,xM+xM+50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 6){
                                    doc.addPage();
                                    doc.fillColor('black').text('Test Title', 20, 50, {});
                                    doc.fillColor('gray').text(testItem.get("data")["name"], 100, 50, {});

                                    doc.fillColor('gray').text('Powered by STAHT CONNECT', 20, doc.page.height - 50, {lineBreak: false});
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'});


                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            //console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log("this error ");
                                            console.log(err);
                                        }

                                        // var rotatedImg = await thenable;

                                        // doc.image(rotatedImg, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});




                                        //  doc.image(im.data, 50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 7){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [0, 0]});
                                    if(isTest){
                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            // console.log(rrr);
                                            doc.image(rotatedImg, xM*1 +(20), yM*0+(80),  {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        //  doc.image(im.data, 50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1 +(20), yM*0+(80), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 8){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rotated ");
                                            // console.log(rotatedImg);
                                            //console.log(rrr);
                                            doc.image(rotatedImg, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }
                                        // doc.image(rotatedImg,   xM*0+(10), yM*1+(90),  {fit: [xM,yM], align: 'center', valign: 'center'});


                                        // doc.image(im.data,xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }



                                }else if(i == 9){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    // doc.rotate(-90,{origin: [  0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+50,-yM-yM-60, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*1+(90), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 10){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //   doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }

                                        // doc.image(im.data,xM+xM+50,-yM-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});
                                    }else{
                                        doc.image(im.data, xM*0+(10), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }else if(i == 11){
                                    var im =  await axios.get(photoList[i], {responseType: 'arraybuffer'})
                                    //  doc.rotate(-90,{origin: [ 0, 0]});
                                    if(isTest){

                                        try{
                                            var rrr = await {
                                                then: function(resolve, _reject) {
                                                    jo.rotate(im.data, options, (error, buffer, orientation, dimensions, quality) => {
                                                        if (error) {
                                                            resolve(im.data)
                                                        }
                                                        resolve(buffer)
                                                    })}};
                                            var rotatedImg =rrr;
                                            console.log("rturned ");
                                            console.log(rotatedImg);
                                            console.log(rrr);
                                            doc.image(rotatedImg,xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                        }catch (err){
                                            console.log(err);
                                        }


                                        //  doc.image(im.data,xM+xM+50,-yM-yM-10-50, {fit: [xM-5,yM], align: 'center', valign: 'center'});

                                    }else{
                                        doc.image(im.data, xM*1+(20), yM*2+(100), {fit: [xM,yM], align: 'center', valign: 'center'});
                                    }


                                }
                                // doc.restore();
                            }

                    }
                }

            }










// for (var j = testItem.get("attachment").length - 1; j >= 0; j--) {
//     var photoCount = 0 ;
//     if(testItem.get("attachment")[j]["type"] == "photo"){
//         photoCount ++;

//           //   doc.fillColor('gray').text((String(i1)), doc.page.width-30,  doc.page.height);







//     if(photoCount == 1){
//         // var im =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//        //  doc.image(im.data, 50, 70, { width: 146 , height : 260,align: "center"});
//         // doc.image(im.data, 200, 70, { width: 146 , height : 260,align: "center"});

//       }

//     // if(photoCount == 2){
//     //      var im2 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im2.data, 200, 70, { width: 146 , height : 260,align: "center"});
//     //   }


//     // if(photoCount == 3){
//     //      var im3 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im3.data, 50, 360, { width: 200 , height : 260,align: "center"});
//     //   }
//     // if(photoCount == 4){
//     //      var im4 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im4.data, 300, 360, { width: 200 , height : 260,align: "center"});
//     //   }

//     // if(photoCount == 5){
//     //      var im5 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im5.data, 50, 420, { width: 200 , height : 260,align: "center"});
//     //   }
//     // if(photoCount == 6){
//     //      var im6 =  await axios.get(testItem.get("attachment")[j]["link"], {responseType: 'arraybuffer'})
//     //      doc.image(im6.data, 300, 420, { width: 200 , height : 260,align: "center"});
//     //   }
//       }

// }
//if(testItem.get("grid")) doc.image(new Buffer(testItem.get("grid").replace('data:image/png;base64,',''), 'base64'), (doc.page.width - 500) /2, 80, { width: 500 ,align: "center"});
//if(testItem.get("grid")) doc.image(new Buffer(testItem.get("grid").replace('data:image/png;base64,',''), 'base64'),20,70, { width: 500 , align: "center"});
        }
        for (var i = filteredAgain.length - 1; i >= 0; i--) {
            await createPage(filteredAgain[i], filteredAgain.length-(2*i));
        }

    }catch(err){
        console.log("Could not get Test items");
        console.log(err);
        // res.send("Could not get Test items");
    }




}


/*
exports.generateReport = (req, res) => {

    res.set('Access-Control-Allow-Origin', '*');
    res.set('Access-Control-Allow-Methods', 'POST');
    res.set('Access-Control-Allow-Headers', 'Content-Type');
    var reportId = req.body.reportId;
    var isTest = true;
    console.log(req.body);
    try{
        // reportId = "kSrPZX5CJaWeM98bCB1d";
        const firestoreDb = admin.firestore();
        // const customerInfo = await firestoreDb.collection('customers').where("report_id", "==", reportId).get();
        firestoreDb.collection("reports").where("report_id", "==", reportId).get().then((querySnapshot) => {
            querySnapshot.docs.map((doc) => firestoreDb.collection("customers").where("projectId", "==", doc.get("resource_id")).get().then((querySnapshotCustomer) => {
                querySnapshotCustomer.docs.map((docCustomer) => createReport(res,docCustomer,reportId,isTest));
            }));
        });

    }catch(err){
        console.log(err);
        res.send(err);

    }

};


 */


var reportId = "uM7hN9ehIzr1j5iqeyka";
var isTest = true;

var res = "";

try{
    // reportId = "kSrPZX5CJaWeM98bCB1d";
    const firestoreDb = admin.firestore();
    // const customerInfo = await firestoreDb.collection('customers').where("report_id", "==", reportId).get();
    firestoreDb.collection("reports").where("report_id", "==", reportId).get().then((querySnapshot) => {
        querySnapshot.docs.map((doc) => firestoreDb.collection("customers").where("projectId", "==", doc.get("resource_id")).get().then((querySnapshotCustomer) => {
            querySnapshotCustomer.docs.map((docCustomer) => createReport(res,docCustomer,reportId,isTest));
        }));
    });

}catch(err){
    console.log(err);
    res.send(err);

}
